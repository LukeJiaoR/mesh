...
void ObjLoader::load(char* filename)
{
    ...
    std::cout << "put in vector "<< Faces.size() << endl;
...
{
    const int Vm =(int)Positions.size();
    const int Fm =(int)Faces.size();
    GLfloat glverts[Vm][3];
    GLint glface[Fm][3];
   // Vec3 *p = &Positions.front();
    for(int i=0; i<(int)Positions.size();++i){


            glverts[i][0] =verts[i].x;
            glverts[i][1] = verts[i].y;
            glverts[i][2] = verts[i].z;
            if(i == Vm-2)
                std::cout << "put in verts array"<< i<< endl;
    }
    for(int j=0; j<(int)Faces.size();++j){

            glface[j][0] = Faces[j].a;
            glface[j][1] = Faces[j].b;
            glface[j][2] = Faces[j].c;
            if(j == Fm-2)
                std::cout << "put in face array"<< j<< endl;
    ...
    //glDrawElements( GL_TRIANGLES,3*Fm,GL_UNSIGNED_INT,glface );
    for(int i =0;i < Fm;i++){

        glColor3f(1.0f,1.0f,1.0f);
        glBegin(GL_TRIANGLES);

        //std::cout << "Success to DRAW GL_TRIANGLES " <<Fm<<endl;
        for(int j  = 0; j<3; j++){

            int k =glface[i][j];
             glVertex3fv(glverts[k]);
